...
of phase settings on the amplifiers.
...
and then whatever output signal was produced from amplifier A.
...
the very first signal and the very last signal.
...
class Day07Spec : Spek({
        ...
        describe("calculate max thruster signal") {
            ...
            onData("intCodes %s phase setting %s", with = *testData) { intCodes, phaseSettings, expected ->
                it("should calculate $expected") {
                    runChainedIntCodes(intCodes, phaseSettings) `should equal` expected
                }
            }
        }
    ...
    describe("part 2") {
        describe("using a channel for input and output") {
            given("intCodes and input") {
                val intCodesString = "3,15,3,16,1002,16,10,16,1,16,15,15,4,15,99,0,0"
                val intCodes = parseIntCodes(intCodesString)
        ...
        describe("calculate max thruster signal asynchronously") {
            val intCodesString = "3,31,3,32,1002,32,10,32,1001,31,-2,31,1007,31,0,33,1002,33,7,33,1,33,31,31,1,32,31,31,4,31,99,0,0,0"
            val intCodes = parseIntCodes(intCodesString)
            val phaseSettings = listOf(1,0,4,3,2)
            on("run asynchronous") {
        ...
        describe("find best phase settings looped (example 1)") {
            val intCodesString = "3,26,1001,26,-4,26,3,27,1002,27,2,27,1,27,26,27,4,27,1001,28,-1,28,1005,28,6,99,0,0,5"
            val intCodes = parseIntCodes(intCodesString)
            it("should find best settings") {
                val result = findBestPhaseSettingsLooped(intCodes)
                result `should equal` (listOf(9,8,7,6,5) to 139629729)
            }
        }
        describe("find best phase settings looped (example 2)") {
            val intCodesString = "3,52,1001,52,-5,52,3,53,1,52,56,54,1007,54,5,55,1005,55,26,1001,54,-5,54,1105,1,12,1,53,54,53,1008,54,0,55,1001,55,1,55,2,53,55,53,4,53,1001,56,-1,56,1005,56,6,99,0,0,0,0,10"
            val intCodes = parseIntCodes(intCodesString)
            it("should find best settings") {
                val result = findBestPhaseSettingsLooped(intCodes)
                result `should equal` (listOf(9,7,8,5,6) to 18216)
            }
        }
        describe("exercise") {
            val intCodesString = readResource("day07Input.txt")!!
            ...
            it("should find best settings") {
                val result = findBestPhaseSettingsLooped(intCodes)
                result `should equal` (listOf(8, 6, 7, 9, 5) to 9246095)
            }
        }
...
fun findBestPhaseSettings(intCodes: List<Int>): Pair<List<Int>, Int> =
    listOf(0, 1, 2, 3, 4).permute().map { phaseSettings ->
        val output = runChainedIntCodes(intCodes, phaseSettings)
        phaseSettings to output
    }.maxBy { it.second }!!

fun runChainedIntCodes(intCodesString: String, phaseSettings: List<Int>): Int =
    runChainedIntCodes(parseIntCodes(intCodesString), phaseSettings)
...
fun findBestPhaseSettingsLooped(intCodes: List<Int>): Pair<List<Int>, Int> =
    listOf(5, 6, 7, 8, 9).permute().map { phaseSettings ->
        val output = runLoopedIntCodes(intCodes, phaseSettings, looped = true)
        phaseSettings to output
    }.maxBy { it.second }!!
